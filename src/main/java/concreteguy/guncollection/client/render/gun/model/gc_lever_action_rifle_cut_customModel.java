...
public class gc_lever_action_rifle_cut_customModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemDisplayContext display, ItemStack stack, ItemStack parent, @Nullable LivingEntity entity, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
        ...
        poseStack.translate(0, 5.8 * 0.0625, 0);
        RenderUtil.renderModel(SpecialModels.GC_LEVER_ACTION_RIFLE_CUT_MAIN.getModel(), display, null, stack, parent, poseStack, buffer, light, overlay);

        poseStack.pushPose();
        poseStack.translate(0, -5.8 * 0.0625, 0);
        poseStack.translate(0, 0, (boltMovement * 2.5) * 0.0625);
        poseStack.translate(0, 5.8 * 0.0625, 0);
        RenderUtil.renderModel(SpecialModels.GC_LEVER_ACTION_RIFLE_CUT_BOLT.getModel(), display, null, stack, parent, poseStack, buffer, light, overlay);
        poseStack.popPose();
        ...
        poseStack.translate(0, 6.9 * 0.0625, -3.1 * 0.0625);
        RenderUtil.renderModel(SpecialModels.GC_LEVER_ACTION_RIFLE_CUT_LEVER.getModel(), display, null, stack, parent, poseStack, buffer, light, overlay);
        poseStack.popPose();
