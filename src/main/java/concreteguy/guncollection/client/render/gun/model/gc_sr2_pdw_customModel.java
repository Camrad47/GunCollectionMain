...
public class gc_sr2_pdw_customModel implements IOverrideModel {
    ...
    {
        BakedModel bakedModel = SpecialModels.GC_SR2_PDW_MAIN.getModel();
        ...
        poseStack.translate(0, 5.8 * 0.0625, 0);
        RenderUtil.renderModel(SpecialModels.GC_SR2_PDW_BOLT.getModel(), display, null, stack, parent, poseStack, buffer, light, overlay);
        poseStack.popPose();


        if(Gun.getAttachment(IAttachment.Type.SCOPE, stack).getItem() == ItemRegistry.GC_SIGHT_RIFLEMAN.get())
            RenderUtil.renderModel(SpecialModels.GC_SR2_PDW_RAIL.getModel(), stack, poseStack, buffer, light, overlay);
