...
public class gc_sks_vssk_customModel implements IOverrideModel {
    ...
    {
        BakedModel bakedModel = SpecialModels.GC_SKS_VSSK_MAIN.getModel();
        ...
        if(Gun.getAttachment(IAttachment.Type.SCOPE, stack).getItem() == ItemRegistry.GC_SIGHT_RIFLEMAN.get())
            RenderUtil.renderModel(SpecialModels.GC_SKS_VSSK_SIGHT_RAIL.getModel(), stack, poseStack, buffer, light, overlay);
        ...
        poseStack.translate(0, 5.8 * 0.0625, 0);
        RenderUtil.renderModel(SpecialModels.GC_SKS_VSSK_BOLT.getModel(), display, null, stack, parent, poseStack, buffer, light, overlay);
        poseStack.popPose();
    }
