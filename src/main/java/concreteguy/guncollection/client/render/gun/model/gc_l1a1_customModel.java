...
public class gc_l1a1_customModel implements IOverrideModel {
    ...
    {
        RenderUtil.renderModel(SpecialModels.GC_L1A1_MAIN.getModel(), stack, poseStack, buffer, light, overlay);

        if(Gun.getAttachment(IAttachment.Type.SCOPE, stack).getItem() == ItemRegistry.GC_SIGHT_RIFLEMAN.get())
            RenderUtil.renderModel(SpecialModels.GC_FAL_RIFLE_SCOPE_BASE.getModel(), stack, poseStack, buffer, light, overlay);

        if(Gun.getAttachment(IAttachment.Type.SCOPE, stack).getItem() == ItemRegistry.GC_SIGHT_OPTIC.get())
            RenderUtil.renderModel(SpecialModels.GC_FAL_SIGHT_RAIL.getModel(), stack, poseStack, buffer, light, overlay);
