...
public class gc_mosin_lucky_carbine_customModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemDisplayContext display, ItemStack stack, ItemStack parent, @Nullable LivingEntity entity, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
        ...
        poseStack.mulPose(Axis.ZN.rotationDegrees(-15F * cooldownSigma));
        poseStack.mulPose(Axis.XN.rotationDegrees(-15F * cooldownSigma));
        poseStack.translate(0, 5.8 * 0.0625, 0);
        RenderUtil.renderModel(SpecialModels.GC_MOSIN_LUCKY_CARBINE_MAIN.getModel(), display, null, stack, parent, poseStack, buffer, light, overlay);
        ...
        poseStack.translate(0, 5.8 * 0.0625, 0);
        RenderUtil.renderModel(SpecialModels.GC_LUCKY_CARBINE_BOLT.getModel(), display, null, stack, parent, poseStack, buffer, light, overlay);
        poseStack.popPose();
    }
