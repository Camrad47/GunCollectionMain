...
public class gc_ak_366_customModel implements IOverrideModel {
    ...
    {
        BakedModel bakedModel = SpecialModels.GC_AK_366_MAIN.getModel();
        ...
        if(!attachmentStack.isEmpty())
        {
            ...
            if(Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ItemRegistry.GC_POLYMER_STOCK.get())
                RenderUtil.renderModel(SpecialModels.GC_AKM_PARTS_POLYMER.getModel(), stack, poseStack, buffer, light, overlay);
            if(Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ItemRegistry.GC_MARKSMAN_STOCK.get())
                RenderUtil.renderModel(SpecialModels.GC_AKM_PARTS_MARKSMAN.getModel(), stack, poseStack, buffer, light, overlay);
        ...
        {
            RenderUtil.renderModel(SpecialModels.GC_AK_366_PARTS_MAIN.getModel(), display, null, stack, parent, poseStack, buffer, light, overlay);
        }
        poseStack.pushPose();
