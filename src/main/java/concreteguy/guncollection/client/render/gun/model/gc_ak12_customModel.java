...
public class gc_ak12_customModel implements IOverrideModel {
    ...
    {
        BakedModel bakedModel = SpecialModels.GC_AK12_MAIN.getModel();
        ...
        if(Gun.getAttachment(IAttachment.Type.BARREL, stack).getItem() == ItemRegistry.GC_MUZZLE_SILENCER_COVER.get()){
            RenderUtil.renderModel(SpecialModels.GC_AK12_MAIN_1.getModel(), stack, poseStack, buffer, light, overlay);}
        else {
            RenderUtil.renderModel(SpecialModels.GC_AK12_MAIN.getModel(), stack, poseStack, buffer, light, overlay);
        }
